...
// One-time migration tool (#242): assign `tagId` to every game whose legacy
// `field` ('platform'|'duration') equals `value`, then clear that legacy field
// on those games. Returns the number of games changed, or null if the round is
// gone. The whole `data` blob is rewritten per game (not a merge) so the legacy
// key is actually removed. Removed once the author has migrated their own data
// (#243). Awaits sequentially — one connection per transaction (tenancy-rls.md).
async function migrateLegacyFieldToTag(tenant, rid, field, value, tagId) {
  return tx(tenant, async (trx) => {
    const round = await trx('rounds').where({ id: rid, tenant_id: tenant }).first('id');
    if (!round) return null;
    ...
    for (const row of rows) {
      const tags = new Set(row.data.tagIds || []);
      tags.add(tagId);
      const data = { ...row.data, tagIds: [...tags] };
      delete data[field];
      await trx('games')
        .where({ id: row.id, round_id: rid, tenant_id: tenant })
        .update({ data: J(data) });
      count++;
    }
    return count;

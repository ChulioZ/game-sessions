...
async function redactText({ kind, roundId, id: targetId }, replacement) {
  ...
  return tx(tenant, async (trx) => {
    ...
    const table = kind === 'game' ? 'games' : 'members';
    const field = kind === 'game' ? 'title' : 'name';
    const row = await trx(table)
      .where({ id: targetId, round_id: roundId, tenant_id: tenant }).first('data');
    if (!row) return null;
    await trx(table).where({ id: targetId, tenant_id: tenant })
      .update({ data: mergeData({ [field]: replacement }) });

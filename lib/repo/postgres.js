 ...
 * which the RLS policies admit in their USING clause only (migration
 * 20260720140000_moderation.js). So a lookup can see across tenants while a
 * WRITE still cannot — WITH CHECK remains strictly tenant-matched. The setting
 * is transaction-local (set_config(..., true)), so it dies at COMMIT and never
 * reaches the next pooled checkout, exactly like app.tenant_id.
 *

...
// background is always present (may be null); tags only when they have ever
// been written (matches the JSON model, where that key is absent until addTag
// runs). tenant_id is scoping metadata, never payload.
...
const ROUND_OBJ = "jsonb_build_object('id', id, 'name', name, 'background', background, 'tags', tags)";
const READ_SQL = {
  // bindings: [tenant]
  list: `
...
async function createRound(tenant, { name, members, importFromRoundId }) {
  return tx(tenant, async (trx) => {
    ...
    const round = await trx('rounds').where({ id: rid }).select('id', 'name', 'background', 'tags');

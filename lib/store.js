...
// One-time migration: the removed veto vote option becomes the lowest rating
// (1). Runs on load; the next saveData() persists it.
function migrate(data) {
  for (const round of data.rounds) {
    // Games created before the duration feature get null (= unknown duration;
    // they are still drawn when all durations are selected).
    for (const g of round.games || []) {
      if (!('duration' in g)) g.duration = null;
    }

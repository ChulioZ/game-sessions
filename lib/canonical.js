...
function createCanonicalRedirect() {
  const canonical = (process.env.CANONICAL_HOST || DEFAULT_CANONICAL).trim().toLowerCase();
  const redirectHosts = new Set(parseHosts(process.env.REDIRECT_HOSTS, DEFAULT_REDIRECT_HOSTS));
  ...
  return function canonicalRedirect(req, res, next) {
    ...
    if (!canonical || redirectHosts.size === 0) return next();
    ...
    if (!host || host === canonical || !redirectHosts.has(host)) return next();
...
module.exports = { createCanonicalRedirect, DEFAULT_CANONICAL, DEFAULT_REDIRECT_HOSTS };
